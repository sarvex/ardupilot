...
#            self.context_clear_collection("STATUSTEXT")
    ...
    def check_attitudes_match(self, a, b):
        ...
        for get_name in get_names:
            self.progress(f"{get_name}: {msgs[get_name]}")
        ...
        euler = q.euler
        self.progress(f"attquat:{str(attitude_quaternion)} q:{q} euler:{euler}")
    ...
    def fly_ahrs2_test(self):
        ...
        self.progress(f"AHRS2: {str(ahrs2)}")
    ...
    def test_rangefinder(self):
        ...
        try:
            ...
            if abs(rf.distance - gpi.relative_alt/1000.0) > 3:
                raise NotAchievedException(
                    f"rangefinder alt ({rf.distance}) disagrees with global-position-int.relative_alt ({gpi.relative_alt / 1000.0})"
                )
            self.wait_statustext("Auto disarmed", timeout=60)
